...
# Loading function
def load_simclrv2(model_name):
    # Models name has to contain string like "r152_3x_sk1"
    depth, width, sk_ratio = name_to_params(model_name)
    model, head = get_resnet(depth=depth, width_multiplier=width, sk_ratio=sk_ratio)
    state_dict = torch.load(os.path.join('X:/Trained_model_storage/SSL Images/checkpoints/simclr_v2s', model_name + '.pth'))
