"""
Split final layer implementation of vision transformers

Majority of Code from: https://github.com/huggingface/pytorch-image-models/blob/main/timm/models/vision_transformer.py
"""
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        print('attn layer size: ', x_res.shape)

        x_res = self.ls1(x_res)
        
        x = x + self.drop_path1(x_res)

        print('combined layer size: ', x.shape)

        x_res = self.norm2(x)
        x_res = self.mlp(x_res)
        x_res = self.ls2(x_res)

        print('mlp size: ', x.shape)

        x = x + self.drop_path2(x_res)

        print('final block size: ', x.shape)
        print('\n')
...
################################################################################
class SplitVisionTransformer(nn.Module):
    def __init__(
            self,
            out_dim_list, 
...
"""
def split_vit_tiny_patch16_224(fc_out, embed_layer=PatchEmbed):
    return SplitVisionTransformer(out_dim_list=fc_out, patch_size=16, embed_dim=192, depth=12, num_heads=3, embed_layer=embed_layer)

def split_vit_small_patch16_224(fc_out, embed_layer=PatchEmbed):
    return SplitVisionTransformer(out_dim_list=fc_out, patch_size=16, embed_dim=384, depth=12, num_heads=6, embed_layer=embed_layer)

def split_vit_base_patch16_224(fc_out, embed_layer=PatchEmbed):
    return SplitVisionTransformer(out_dim_list=fc_out, patch_size=16, embed_dim=768, depth=12, num_heads=12, embed_layer=embed_layer)
...
# data_batch = torch.rand(size=(100, 3, 224, 224))#.to('cuda')
# model = split_vit_base_patch16_224([500, 200, 300])#.to('cuda')
# print(model.forward(data_batch, 'all').shape)
