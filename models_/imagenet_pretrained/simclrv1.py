...
# Load simclr v1 model
def load_simclrv1(model_name):
    if model_name == 'simclr-v1-resnet50-1x':
        model = resnet50x1()
    elif model_name == 'simclr-v1-resnet50-2x':
        model = resnet50x2()
    elif model_name == 'simclr-v1-resnet50-4x':
        model = resnet50x4()

    state_dict = torch.load(os.path.join('X:/Trained_model_storage/SSL Images/checkpoints/simclr_v1s', model_name + '.pth'), map_location='cpu')
